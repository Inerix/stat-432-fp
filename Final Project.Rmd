...
```{r, read-data, data-wrangling, message = FALSE}
data = read.csv("/Users/Carrie/Desktop/song_data.csv")

# Eliminate song_name from the dataset
song_data = data %>% select(-c(song_name, energy))
...
set.seed(42)
# test-train split
tst_trn_split = initial_split(song_data, prop = 0.80)
song_trn = training(tst_trn_split)
song_tst = testing(tst_trn_split)
# estimation-validation split
est_val_split = initial_split(song_trn, prop = 0.80)
song_est = training(est_val_split)
song_val = testing(est_val_split)
...
song_est_x = model.matrix(song_popularity~ ., data = song_est)[, -1]
song_val_x = model.matrix(song_popularity~ ., data = song_val)[, -1]
...
```{r, ridge-regression, echo = TRUE, message = FALSE, warning = FALSE}
set.seed(42)
...
```{r, Xgboost-model, echo = TRUE, message = FALSE, warning = FALSE}
...
pred_lm_mod1 = predict(lm_mod1, song_val)
lm_mod1_rmse = calc_rmse(actual = song_val$song_popularity, predicted = pred_lm_mod1)

pred_lm_mod2 = predict(lm_mod2, song_val)
lm_mod2_rmse = calc_rmse(actual = song_val$song_popularity, predicted = pred_lm_mod2)
```
...
tibble(
  ...
  "Validation RMSE" = c(lm_mod1_rmse,
                        lm_mod2_rmse,
                        glm_rmse,
                        rf_rmse,
                        glmnet_rmse,
...
After evaluation, the chosen model, a random forest model, achieves a test RMSE of `r rf_mod_tst_rmse`. This error is rather large and makes predictions relatively volatile. Additionally, due to class imbalance, a lot of the predictions are centered around 40-60 range. By examining the actual versus predicted plot of the validation data, we see that song popularity predicted to be 50 can actually be 0. Given these errors, it should be noted that this model is not particularly useful in predicting song popularity. More data could be gathered to train models, and class imbalance problem should be addressed in the future. 
