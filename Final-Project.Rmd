...
After evaluation, the chosen model, a random forest model, achieves a test RMSE of `r rf_mod_tst_rmse`. This error is rather large and makes predictions relatively volatile. Additionally, due to class imbalance, a lot of the predictions are centered around 40-60 range. By examining the actual versus predicted plot of the validation data, we see that our model tends to *overpredict* popularity for songs with actual popularity less than 50, and *underpredict* popularity for songs with actual popularity greater than 50. We also see that the variance in our predictions decrease as actual popularity increases; the most popular songs are still being predicted with quite low error. 
